...
from csnn import set_sym_type, Linear, Sequential, ReLU, Softplus, RNN
from models.complementarity_RNN import ComplementarityRNN
from models.inverted_pendulum import InvertedPendulum


class InvertedPendulumRNN:
    """MPC approximation with RNN for inverted pendulum system.
    
    This class sets up and solves an optimization problem to learn a neural network
    policy, using a RNN, for inverted pendulum control using multiple parallel shooting with collocation,
    ...
    def __init__(
        ...
        if model_dir is None:
            self.model_dir = Path(__file__).parent.parent / "models_nn" / "rnn_inverted_pendulum"
        else:
    ...
    def setup_network(self):
        ...
        for layer_idx, size in enumerate(self.hidden_sizes):
            ...
            result = rnn.build(hidden_seq, h0=h0, params=params_layer, tau=self.tau)
    ...
    def setup_complementarity_network(self):
        ...
        for layer_idx, size in enumerate(self.hidden_sizes):
            ...
            self.rnns.append(rnn)
            cc_vars.extend(result["vars"])
            cc_lbw.extend(result["lbw"])
            cc_ubw.extend(result["ubw"])
            cc_g.extend(result["g"])
            cc_lbg.extend(result["lbg"])
            cc_ubg.extend(result["ubg"])
        ...
        self.n_param = int(self.params_flattened.shape[0])
        self.net = output_seq
        self.cc_vars = cc_vars
        self.cc_vars_lbw = cc_lbw
        self.cc_vars_ubw = cc_ubw
        self.cc = cc_g
        self.cc_lbg = cc_lbg
        self.cc_ubg = cc_ubg
    ...
    def network_warm_start_with_sgd(self, params_init, learning_rate=1e-3, num_samples=20, iterations=500):
        ...
        for i in range(num_samples):
            u_opt = self.inverted_pend.solve_MPC(X_train[:, i], ret_seq=True)
