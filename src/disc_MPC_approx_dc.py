...
from csnn import set_sym_type, Linear, Sequential, ReLU, Softplus, Sigmoid
from models.discontinuous_control import DiscontinuousControl


class DiscontinuousMPCInputCollocation:
    """MPC approximation with input collocation for discontinuous control law.
    
    This class sets up and solves an optimization problem to learn a neural network
    policy for discontinuous control using multiple parallel shooting with collocation,
    ...
    def __init__(
        ...
        layer_sizes : list of int
            Neural network layer sizes (input must be 2 for state).
        batch_size : int
        ...
        model_dir : Path or str, optional
            Directory to save model parameters. Defaults to ./models_nn.
        """
        # Store configuration
        ...
        # State bounds
        self.state_bounds = np.array([1000.0, 1000.0])
        
        # Training bounds
        alpha = 1e-3
        self.train_bounds = alpha * self.state_bounds
        ...
        if model_dir is None:
            self.model_dir = Path(__file__).parent.parent / "models_nn" / "discontinuous_input"
        else:
        ...
        self.sys = DiscontinuousControl(dt=0.1, sym_type='SX')
    ...
    def generate_initial_states(self):
        ...
        # Sample points uniformly in the unit circle
        r = 1.0
        theta = np.random.uniform(-np.pi, np.pi, self.NB)
        x = r * np.cos(theta)
        y = r * np.sin(theta)
        self.X_train = np.vstack([x, y])
    ...
    def network_warm_start_with_sgd(self, params_init, learning_rate=1e-3, num_samples=20, iterations=500):
        ...
        for i in range(num_samples):
            u_opt = self.sys.solve_MPC(X_train[:, i])
    ...
    def setup_optimization(self, params_init=None, warm_start='mpc'):
        ...
        # A, B = self.sys.lin_dyn(self.xr, self.ur)
        # E = solve_continuous_are(A.full(), B.full(), self.Q.full(), self.R.full())
        E_dm = ca.DM.zeros(self.NX, self.NX)
        ...
        state_lb = -self.state_bounds
        state_ub = self.state_bounds
    ...
    def save_results(self):
        ...
        # Save YAML
        yaml_path = self.model_dir / f"optimal_params_disc_{self.model_name}_{date_str}.yaml"
