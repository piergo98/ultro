...
NN_params = ca.MX.sym('NN_params', n_param)
w += [NN_params]
lbw += [-20.0]*n_param
ubw += [ 20.0]*n_param
...
for i in range(NB):
    ...
    w0 += x0
    for k in range(N):
        u_k = ca.MX.sym('U_' + str(i) + '_' + str(k), NU)
        w += [u_k]
        lbw += [-1.0]*NU
        ubw += [1.0]*NU
        w0 += [0.0]*NU
        
        # Add constraint on control input to be given by the NN approximator
        g += [u_k - net_fcn(Xk, NN_params)]
        lbg += [0.0]*NU
        ubg += [0.0]*NU
