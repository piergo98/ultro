...
cart_pole = CartPole(sym_type='MX')

# Declare variables
NX = 4      # state dimension
NU = 1      # control dimension
NB = 80     # batch size for learning
N = 30      # horizon length
...
for i in range(NB):
    ...
    NN_params = ca.MX.sym('theta_' + str(i), n_param)  # parameters to optimize
    w += [NN_params]
    lbw += [-50.0]*n_param
    ubw += [ 50.0]*n_param
    ...
    for k in range(N):
        ...
        lr = 1e-3
        # Penalize the parameters across batches to avoid overfitting
...
# Extract the optimal parameters and trajectories for all batches
w_opt = solution['x'].full().flatten()

# Structure: for each batch i: [theta_i, X_i_0, X_i_1, ..., X_i_N]
# Block size per batch: n_param + (N+1)*NX
block_size_per_batch = n_param + (N + 1) * NX

# Storage
theta_opt = np.zeros((NB, n_param))  # parameters for each batch
...
for i in range(NB):
    ...
    # Compute controls using learned parameters
    for k in range(N):
        xk_dm = ca.DM(x_opt[i, k, :])
        uk = net_fcn(xk_dm, theta_opt[i, :])
        u_opt[i, k] = float(uk.full().item())

# Use the mean of all batch parameters as the "optimal" parameters
optimal_params = theta_opt.mean(axis=0)

# Print diagnostics about parameter variation across batches
param_std = theta_opt.std(axis=0)
param_range = theta_opt.max(axis=0) - theta_opt.min(axis=0)
print(f"\nParameter statistics across {NB} batches:")
print(f"  Mean parameter L2 norm: {np.linalg.norm(optimal_params):.4f}")
print(f"  Avg std dev per param: {param_std.mean():.4f}")
print(f"  Max std dev per param: {param_std.max():.4f}")
print(f"  Avg range per param: {param_range.mean():.4f}")
print(f"  Max range per param: {param_range.max():.4f}")
...
params_dict = {
    "optimal_params": np.asarray(optimal_params).tolist(),  # mean across batches
    "optimal_params_all_batches": np.asarray(theta_opt).tolist(),  # all batch parameters (NB x n_param)
    "param_std": np.asarray(param_std).tolist(),  # std dev per parameter
    "n_param": int(n_param),
...
# Save JSON
json_path = MODEL_DIR / f"optimal_params_cp_{model_name}_{date_str}.json"
with json_path.open("w") as f:
    json.dump(params_dict, f, indent=2)
print(f"Saved parameters to {json_path}")

# Save YAML
yaml_path = MODEL_DIR / f"optimal_params_cp_{model_name}_{date_str}.yaml"
