...
class CartPoleMPCInputCollocation:
    """MPC approximation with input collocation for cart pole system.
    
    This class sets up and solves an optimization problem to learn a neural network
    policy for cart pole control using multiple parallel shooting with collocation,
    ...
    def __init__(
        ...
        self.cart_pole = CartPole(sym_type='MX')
    ...
    def network_warm_start_with_sgd(self, params_init, learning_rate=1e-3, iterations=500):
        ...
        X_train = np.random.uniform(
            low=[-self.p_train_bound, -self.v_train_bound, -self.theta_train_bound, -self.omega_train_bound],
            high=[self.p_train_bound, self.v_train_bound, self.theta_train_bound, self.omega_train_bound],
            size=(num_samples, self.NX)
        )
        
        # Define surrogate loss (e.g., MSE to zero control)
        def surrogate_loss(params):
            u_pred = np.array([self.net_fcn(X_train[i], params.reshape(-1, 1)).full().flatten() for i in range(num_samples)])
            return np.mean(u_pred**2) + self.regularization * np.linalg.norm(params)**2
        ...
        for it in range(iterations):
            ...
            for i in range(len(params)):
                ...
                grad[i] = (surrogate_loss(params_eps_plus) - surrogate_loss(params_eps_minus)) / (2 * epsilon)
            
            # Update parameters
            params -= learning_rate * grad
            
            if (it + 1) % 100 == 0:
                current_loss = surrogate_loss(params)
                print(f"SGD iteration {it + 1}/{iterations}, surrogate loss: {current_loss:.6f}")
    ...
    def setup_optimization(self, params_init=None):
        ...
        for i in range(self.NB):
            ...
            for k in range(self.N):
                ...
                for j in range(self.degree):
                    Xkj = ca.MX.sym('X_' + str(i) + '_' + str(k) + '_' + str(j), self.NX)
                    Xc.append(Xkj)
                    w += [Xkj]
                ...
                for j in range(1, self.degree + 1):
                    ...
                    J += B_coeff[j] * qj * self.cart_pole.dt
        ...
        J += self.regularization * ca.norm_2(z)
