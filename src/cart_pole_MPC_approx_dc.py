...
class CartPoleMPCInputCollocation:
    ...
    def solve_closed_loop_MPC_for_initial_states(self):
        ...
        for i in range(self.NB):
            ...
            x_traj, u_traj = self.cart_pole.close_loop_simulation(self.X_train[:, i], plot_results=False)
            # Store trajectories
    ...
    def generate_state_warm_start(self, X0):
        ...
        tau_root = np.append(0, ca.collocation_points(self.degree, 'legendre'))
        ...
        for i in range(self.NB):
            ...
            for k in range(self.N):
                ...
                for j in range(self.degree):
                    ...
                    tau = tau_root[j + 1]
                    dt_frac = tau * self.cart_pole.dt
                    
                    # Simple forward Euler from interval start
                    x_dot_val = self.cart_pole.dynamics(x_current, u_zero).full()
                    x_colloc = x_current + dt_frac * x_dot_val
                    
                    state_warm_start[(i, k, j)] = x_colloc.flatten()
                
                # Compute state at end of interval using RK4
                x_next = self.rk4_step(x_current, u_zero, self.cart_pole.dynamics, self.cart_pole.dt)
                x_current = x_next
    ...
    def setup_optimization(self, params_init=None, warm_start='mpc'):
        ...
        for i in range(self.NB):
            x0 = self.X_train[:, i]
            x_target = np.array([0.0, 0.0, 0.0, 0.0])  # Target state (origin)
            ...
            # Define a neural network function for each batch element
            # z_i = ca.SX.sym('z_' + str(i), self.n_param)
            # w += [z_i]
            # lbw += [-50.0] * self.n_param
            # ubw += [50.0] * self.n_param
            # w0 += params_init.tolist()
